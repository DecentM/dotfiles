...
export type ToolExecutionRepository = Repository<ToolExecution> &
	ToolExecutionRepositoryExtension;

/**
 * Get the extended ToolExecution repository.
 */
export const getToolExecutionRepository =
	async (): Promise<ToolExecutionRepository> => {
		...
		return baseRepository.extend<ToolExecutionRepositoryExtension>({
			...
			async getToolStats(): Promise<ToolStats> {
				try {
					...
					return {
						totalExecutions: Number.parseInt(
						...
						avgDurationMs: result?.avgDurationMs
							? Number.parseFloat(result.avgDurationMs)
							: null,
					};
				} catch {
					return {
				...
				try {
					...
					return results.map((row) => ({
						...
						avgDurationMs: row.avgDurationMs
							? Number.parseFloat(row.avgDurationMs)
							: null,
					}));
				} catch {
					return [];
