...
const createTestDb = (): TestDb => {
  ...
  const getSessionLogs = (sessionId?: string, limit = 1000): SessionLogRow[] => {
    const conditions: string[] = []
    const params: (string | number)[] = []
...
describe('database initialization', () => {
  ...
  describe('table creation', () => {
    test('creates tool_execution_log table', () => {
      const result = testDb.db
        .query(`SELECT name FROM sqlite_master WHERE type='table' AND name='tool_execution_log'`)
        .get()
    ...
    test('creates session_log table', () => {
      const result = testDb.db
        .query(`SELECT name FROM sqlite_master WHERE type='table' AND name='session_log'`)
        .get()
      expect(result).toBeDefined()
    })

    test('tool_execution_log has correct columns', () => {
      const columns = testDb.db.query(`PRAGMA table_info(tool_execution_log)`).all() as Array<{
        name: string
      }>
    ...
    test('session_log has correct columns', () => {
      const columns = testDb.db.query(`PRAGMA table_info(session_log)`).all() as Array<{
        name: string
      }>
      const columnNames = columns.map((c) => c.name)
  ...
  describe('index creation', () => {
    ...
    test('creates session_log indexes', () => {
      const indexes = testDb.db
        .query(`SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='session_log'`)
        .all() as Array<{ name: string }>
      const indexNames = indexes.map((i) => i.name)
