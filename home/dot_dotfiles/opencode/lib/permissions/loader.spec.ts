...
`;
			...
			expect(config.rules[0].constraints).toEqual([
				{ type: "cwd_only", value: ["/tmp"] },
			]);
		});
...
`,
			...
			expect(calls.some((c) => c[0].includes("Invalid permissions YAML"))).toBe(
				true,
			);
		});
...
`,
			...
			expect(
				config.rules[0].compiledRegex.test(
					"prefix" + "a".repeat(1000) + "suffix",
				),
			).toBe(true);
		});
