...
const TOOLS = [
  {
    ...
    description: "Fetch a URL with Cloudflare bypass using Flaresolverr. Returns the HTML content, cookies, and headers after bypassing Cloudflare protection.",
    inputSchema: {
      type: "object",
      properties: {
        url: {
        ...
        disableMedia: {
          type: "boolean",
          description: "Disable loading of media (images, CSS, fonts) for faster responses (default: false)",
        },
      },
      required: ["url"],
    },
  },
  {
    ...
    description: "Send a POST request with Cloudflare bypass using Flaresolverr. Returns the HTML content, cookies, and headers after bypassing Cloudflare protection.",
    inputSchema: {
      type: "object",
      properties: {
        url: {
  ...
  },
  {
    ...
    description: "Create a persistent browser session in Flaresolverr. Sessions maintain cookies and browser state across requests.",
    inputSchema: {
      type: "object",
      properties: {
        session: {
...
async function handleGet(args) {
  const { url, session, maxTimeout, returnScreenshot, returnOnlyCookies, proxy, disableMedia } = args;
